...
from pylab import (clf, figure, grid, plot, savefig, show, subplot, title,
...
def make_linear(f0, t1, f1, filename=None, fig_size=FIG_SIZE):
    ...
    subplot(2, 1, 1)
    plot(t, w)
    tstr = "Linear Chirp, $f(0)={:g}, f({:g})={:g}$".format(f0, t1, f1)
    title(tstr)
...
def make_quadratic_v0false(f0, t1, f1, filename=None, fig_size=FIG_SIZE):
    ...
    subplot(2, 1, 1)
    plot(t, w)
    tstr = "Quadratic Chirp, f(0)=%g, f(%g)=%g (vertex_zero=False)" % (f0, t1,
                                                                       f1)
    title(tstr)
